...
from mquake.core.coordinates import CoordinateSystem, Coordinates
import h5py
...
def read_pickle(filename, protocol=-1, **kwargs):
    ...
    :type filename: str
    :rtype: ~mquake.core.data.grid.Grid
    """
    import pickle
...
def write_pickle(grid, filename, protocol=-1, **kwargs):
    ...
    :type grid: ~mquake.core.data.grid.GridData
...
def read_hdf5(filename, **kwargs):
    """
    read a grid file in hdf5 into a mquake.core.data.grid.GridCollection
    object
    :param filename: filename
    :param kwargs: additional keyword argument passed from wrapper.
    :return: mquake.core.data.grid.GridCollection
    """
    pass
...
def write_csv(grid, filename, **kwargs):
    """
    Write a GridData object to disk in mquake csv format
    :param grid: grid to be saved
    :param filename: full path to file with extension
    :return:
    """
...
def read_csv(filename, *args, **kwargs):
    """
    Read a grid save in mquake CSV format
