...
#   Author: mquake development team
#    Email: devs@mquake.org
#
# Copyright (C) 2016 mquake development team
# --------------------------------------------------------------------
"""
Expansion of the obspy.core.event module

:copyright:
    mquake development team (devs@mquake.org)
:license:
...
from mquake.core.coordinates import Coordinates, CoordinateSystem
from mquake.core.util.decorators import update_doc
import json
...
class Catalog(obsevent.Catalog):
    ...
    def write(self, fileobj, format='quakeml', **kwargs):
        ...
        for event in self.events:
            if event_type_lookup.is_valid_mquakeml(event.event_type):
...
class EventTypeLookup(object):
    ...
    def convert_to_quakeml(self, mquakeml_type):
        return self.lookup_table[mquakeml_type]
    ...
    @property
    def valid_mquakeml_types(self):
        return self.lookup_table.keys()

    def is_valid_quakeml(self, event_type):
        return event_type in self.valid_quakeml_types

    def is_valid_mquakeml(self, event_type):
        return event_type in self.valid_mquakeml_types
...
class Event(obsevent.Event):
    ...
    # __doc__ = obsevent.Event.__doc__.replace('obspy', 'mquake')
...
class Origin(obsevent.Origin):
    ...
    # original_doc = obsevent.Origin.__doc__.replace('obspy', 'mquake')
    ...
    __new_section__ = {
        'doc': """:type coordinates: :class:`~mquake.core.coordinates.Coordinates`
    :param coordinates: Spatial coordinates of the event origin.
    """,
        'previous_parameter': 'time_fixed'}

    __example__ = """
    >>> from mquake.core.event import Origin
    >>> from mquake.core.coordinates import Coordinates, CoordinateSystem
    >>> from mquake.core import UTCDateTime
...
class Magnitude(obsevent.Magnitude):
    __doc__ = obsevent.Magnitude.__doc__.replace('obspy', 'mquake')
...
class Pick(obsevent.Pick):
    original_doc = obsevent.Pick.__doc__.replace('obspy', 'mquake')
...
class WaveformStreamID(obsevent.WaveformStreamID):
    __doc__ = obsevent.WaveformStreamID.__doc__.replace('obspy', 'mquake')
...
class Arrival(obsevent.Arrival):
    __doc__ = obsevent.Arrival.__doc__.replace('obspy', 'mquake')
...
def read_events(filename, **kwargs):
    ...
    # converting the obspy object into mquake objects
...
def _set_attr_handler(self, name, value, namespace=namespace):
    """
    Generic handler to set attributes for mquake objects
    ...
    # recursive parse of 'extra' args when constructing mquake from obspy
    elif name == 'extra':
...
def _init_from_obspy_object(mquake_obj, obspy_obj):
    """
    When initializing mquake object from obspy_obj
    checks attributes for lists of obspy objects and
    converts them to equivalent mquake objects.
    """
    ...
    for key, val in obspy_obj.__dict__.items():
        ...
        if itype in class_equiv:
            mquake_obj.__setattr__(key, class_equiv[itype](val))
